...
def lsqfit(exp_times, light_curve, errors, wavelengths,
    ...
    # If you are doing a poly fit or variable detrend,
    # use a quick numpy polyfit to improve the coefficient estimates.
    for i,key in enumerate(list(systematics.keys())):
        if systematics[key]["poly"]:
            ...
            if (save_guess_plot or show_guess_plot):
                ...
                if save_guess_plot:
                    if is_spec:
                        plt.savefig(os.path.join(plot_dir,"s5_"+outfile+"detector{}".format(key)+"_spec{}LSQ_system-planetflare.png".format(wavestr)),
                                    dpi=300, bbox_inches='tight')
                    else:
                        plt.savefig(os.path.join(plot_dir,"s5_"+outfile+"detector{}".format(key)+"_broadbandLSQ_system-planetflare.png"),
                                    dpi=300, bbox_inches='tight')
                if show_guess_plot:
                    plt.show(block=True)
                plt.close()
        for detrend_type, detrend_variable in zip(("disp","spatial","width"),
            ...
            if systematics[key][f"{detrend_type}_detrend"]:
                ...
                if (save_guess_plot or show_guess_plot):
                    ...
                    if save_guess_plot:
                        if is_spec:
                            plt.savefig(os.path.join(plot_dir,"s5_"+outfile+"detector{}".format(key)+"_spec{}LSQ_{}-estimate.png".format(wavestr,detrend_type)),
                                        dpi=300, bbox_inches='tight')
                        else:
                            plt.savefig(os.path.join(plot_dir,"s5_"+outfile+"detector{}".format(key)+"_broadbandLSQ_{}-estimate.png".format(detrend_type)),
                                        dpi=300, bbox_inches='tight')
                    if show_guess_plot:
                        plt.show(block=True)
                    plt.close()
    ...
    results = minimize(fit_handler._residuals,
                       x0=params_array,
                       args=(exp_times, light_curve, errors, bundled_params, priors_types, fit_or_not,
...
# i adapted lsqfit to work on one or many arbitrarily
